-- Ensure termination updates run with actor identity in request claims so
-- audit triggers that depend on auth.uid() do not fail under service-role RPC.
...
BEGIN
  ...
  -- Make auth.uid() available for row update triggers (e.g. audit logs) even
  -- when invoked through service-role edge function calls.
